...
class IndexMain {
    ...
    windowEvent() {
        ...
        window.onhashchange = function(){
            ...
            if(self.chartClass[hash]) {
                // 如果类目录已经存在，则直接初始化
                self.detail.callChartBySide(type);
            } else {
                // 如果类目录不存在，则先引入相应的js
                self.introductionJS(hash);
            };
        };
    ...
    introductionJS(fileName:string) {
        ...
        // 初始化对象
        function initObject(obj:IndexMain) {
            obj.chartClass[fileName] = window["Process"];
            obj.detail.callChartBySide(window["Process"]);
            // 清空缓存
            window["Process"] = null;
        };
    }
...
class CDetail {
    ...
    callChartBySide(chartClass:any) {
        ...
        if(chartClass.completeHtml) {   // 如果html文件载入完毕，则直接调用fetch
            this.currentChart.fetch();
        } else {    // 如果html文件未加载，则先加载文件
            this.introductionHtml(this.currentChart);
        };
    }
...
class ChartBase {
    ...
    static completeHtml:boolean = false; // 是否载入html完成，默认为false
